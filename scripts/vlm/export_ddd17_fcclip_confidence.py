...
from utils.fcclip_confidence_utils import save_confidence_map


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Export offline FC-CLIP pixel-confidence maps for DDD17 frames')
...
def generate_for_split(args: argparse.Namespace) -> None:
    ...
    print(
        f'Generating DDD17 confidence maps for split={args.split} dirs={[path.name for path in split_dirs]} '
    ...
    for sequence_dir in split_dirs:
        ...
        for reference_mask_path in tqdm(reference_masks, desc=sequence_dir.name):
            confidence_path = output_artifact_path(split_output_root, sequence_dir, reference_mask_path)
            if confidence_path.exists() and not args.overwrite:
                continue

            frame_path = resolve_frame_path(sequence_dir, reference_mask_path)
            image = read_image(str(frame_path), format='BGR')
            _, confidence = predictor.predict_mask_and_confidence(image)
            confidence = align_prediction_to_reference(confidence, reference_mask_path)
            save_confidence_map(confidence_path, confidence)
            written += 1

    print(f'WROTE_CONFIDENCE split={args.split} count={written} output_root={split_output_root}')
