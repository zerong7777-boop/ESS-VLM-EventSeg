...
class BaseTrainer(object):
    ...
    def createDDD17EventsDataset(self, dataset_name, root, split_train, batch_size, nr_events_data, delta_t_per_data,
                                 ...
                                 normalize_event, fixed_duration, pseudolabels_path="",
                                 pseudolabel_ignore_label=255, semseg_num_classes=6, confidence_path="",
                                 top2_soft_path="", max_train_samples=None, max_val_samples=None):
        """
        Creates the validation and the training data based on the provided paths and parameters.
        """
        dataset_builder = self.getDataloader(dataset_name)
        ...
        train_dataset = dataset_builder(root=root,
                                        ...
                                        pseudolabels_path=pseudolabels_path if self.settings.use_pseudolabels_train_b else "",
                                        pseudolabel_ignore_label=pseudolabel_ignore_label,
                                        semseg_num_classes=semseg_num_classes,
                                        confidence_path=confidence_path if self.settings.use_confidence_weighted_pseudolabel_loss else "",
                                        top2_soft_path=top2_soft_path if self.settings.use_top2_soft_distillation else "",
    ...
    def combineDataloaders(self):
        ...
        self.train_loader = WrapperDataset(
            self.train_loader_sensor_a,
            self.train_loader_sensor_b,
            self.device,
            dataset_len_to_use=self.settings.train_loader_len_source,
        )
    ...
    def createDataLoaders(self):
        ...
        if self.settings.dataset_name_b == 'DSEC_events':
            ...
            out = self.createDDD17EventsDataset(self.settings.dataset_name_b,
                                                ...
                                                self.settings.normalize_event_b,
                                                self.settings.fixed_duration_b,
                                                self.settings.pseudolabels_path_b,
                                                self.settings.pseudolabel_ignore_label_b,
                                                self.settings.semseg_num_classes,
                                                self.settings.pseudolabel_confidence_path_b,
                                                self.settings.top2_soft_labels_path_b,
                                                self.settings.max_train_samples_b,
                                                self.settings.max_val_samples_b)
    ...
    def train(self):
        ...
        for _ in tqdm(range(self.epoch_count, self.settings.num_epochs), total=self.settings.num_epochs,
            ...
            if (self.epoch_count % val_epoch_step) == 0 and not self.settings.skip_validation:
        ...
        if not self.settings.skip_validation:
            self.validationEpochs()
