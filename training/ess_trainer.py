...
from utils.vpr_reliability import class_reliability_map, normalize_class_reliability
...
class ESSModel(training.base_trainer.BaseTrainer):
    ...
    def init_fn(self):
        ...
        self.vpr_class_weights = None
        if self.settings.use_vpr_pseudo_ce:
            raw_reliability = self.settings.vpr_class_reliability
            if raw_reliability is None:
                raw_reliability = [1.0] * self.settings.semseg_num_classes
            self.vpr_class_weights = normalize_class_reliability(
                raw_reliability,
                min_weight=self.settings.vpr_min_class_weight,
                max_weight=self.settings.vpr_max_class_weight)
            if self.vpr_class_weights.numel() != self.settings.semseg_num_classes:
                raise ValueError('vpr_class_reliability must match semseg_num_classes')
    ...
    def event_train_step(self, batch):
        data_b = batch[1][0]
        pseudo_labels_b = None
        pseudo_confidence_b = None
        pseudo_top1_b = None
        pseudo_top2_b = None
        pseudo_p1_b = None
        pseudo_p2_b = None
        if self.settings.use_pseudolabels_train_b:
            pseudo_labels_b = batch[1][1]

        if self.settings.require_paired_data_train_b:
            if self.settings.use_pseudolabels_train_b:
                label_index = 2
                if self.settings.use_top2_soft_distillation:
                    pseudo_top1_b = batch[1][2]
                    pseudo_top2_b = batch[1][3]
                    pseudo_p1_b = batch[1][4]
                    pseudo_p2_b = batch[1][5]
                    label_index = 6
                elif self.settings.use_confidence_weighted_pseudolabel_loss:
                    pseudo_confidence_b = batch[1][2]
                    label_index = 3
                labels_b = batch[1][label_index]
            else:
                labels_b = batch[1][1]
        else:
            labels_b = batch[1][1]
        ...
        if self.settings.train_on_event_labels:
            ...
            t_loss += t_loss_b
        if self.settings.use_pseudolabels_train_b:
            if self.settings.use_confidence_weighted_pseudolabel_loss:
                t_loss_pseudo, _ = self.trainConfidenceWeightedPseudoStep(
                    'sensor_b_pseudo',
                    latent_real,
                    pseudo_labels_b,
                    pseudo_confidence_b,
                ...
                t_loss_pseudo, _ = self.trainTaskStep('sensor_b_pseudo', latent_real, pseudo_labels_b, losses)
            t_loss_pseudo_weighted = t_loss_pseudo * self.settings.weight_pseudolabel_loss
            losses['semseg_sensor_b_pseudo_weighted_loss'] = t_loss_pseudo_weighted.detach()
            t_loss += t_loss_pseudo_weighted
            if self.settings.use_top2_soft_distillation:
                t_loss_top2 = self.trainTop2SoftPseudoStep(
                    'sensor_b_top2_soft',
                    latent_real,
                    pseudo_labels_b,
                    pseudo_top1_b,
                ...
                t_loss_top2_weighted = t_loss_top2 * self.settings.weight_top2_soft_distillation
                losses['semseg_sensor_b_top2_soft_weighted_loss'] = t_loss_top2_weighted.detach()
                t_loss += t_loss_top2_weighted
            if self.settings.use_vpr_pseudo_ce:
                t_loss += self.trainVPRPseudoCEStep(latent_real, pseudo_labels_b, losses)

        return e_loss, t_loss, losses, out

    def trainConfidenceWeightedPseudoStep(self, sensor_name, latent_fake, pseudo_labels, confidence_map, losses):
        content_features = {}
        for key in latent_fake.keys():
            if self.settings.dataset_name_b == 'DDD17_events':
                content_features[key] = latent_fake[key]
        ...
        task_backend = self.models_dict["back_end"]
        pred = task_backend(content_features)
        loss_pred = confidence_weighted_cross_entropy(
            pred[1],
            pseudo_labels.long(),
            confidence_map.float(),
            ignore_index=self.settings.semseg_ignore_label,
        )
        losses['semseg_' + sensor_name + '_confidence_ce_loss'] = loss_pred.detach()

        return loss_pred, pred

    def trainTop2SoftPseudoStep(self, sensor_name, latent_fake, pseudo_labels, top1_id, top2_id, p1, p2, losses):
        content_features = {}
        for key in latent_fake.keys():
            if self.settings.dataset_name_b == 'DDD17_events':
                content_features[key] = latent_fake[key]
        ...
        task_backend = self.models_dict["back_end"]
        pred = task_backend(content_features)
        valid_mask = pseudo_labels.long() != self.settings.semseg_ignore_label
        loss_pred = top2_soft_cross_entropy(
            pred[1],
            top1_id.long(),
            top2_id.long(),
            p1.float(),
            ...
            temperature=self.settings.top2_soft_temperature,
            min_p1=self.settings.top2_soft_min_p1,
            min_margin=self.settings.top2_soft_min_margin,
        )
        losses['semseg_' + sensor_name + '_loss'] = loss_pred.detach()

        return loss_pred

    def trainVPRPseudoCEStep(self, latent_real, pseudo_labels, losses):
        task_backend = self.models_dict["back_end"]
        pred = task_backend(latent_real)
        logits = pred[1]
        pseudo_labels = pseudo_labels.long()

        ce = f.cross_entropy(logits, pseudo_labels, ignore_index=self.settings.semseg_ignore_label, reduction='none')
        class_weights = self.vpr_class_weights.to(device=pseudo_labels.device)
        pixel_weights = class_reliability_map(pseudo_labels, class_weights,
                                              ignore_index=self.settings.semseg_ignore_label)
        weight_sum = pixel_weights.sum().clamp_min(1.0)
        vpr_loss = (ce * pixel_weights).sum() / weight_sum
        vpr_loss_weighted = vpr_loss * self.settings.weight_vpr_pseudo_ce

        losses['semseg_sensor_b_vpr_ce_loss'] = vpr_loss.detach()
        losses['semseg_sensor_b_vpr_weighted_ce_loss'] = vpr_loss_weighted.detach()

        return vpr_loss_weighted
