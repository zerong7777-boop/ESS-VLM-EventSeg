...
class Settings:
    def __init__(self, settings_yaml, generate_log=True):
        ...
        with open(settings_yaml, 'r') as stream:
            ...
            self.pseudolabels_path_b = ''
            self.use_pseudolabels_train_b = False
            self.pseudolabel_ignore_label_b = 255
            self.pseudolabel_confidence_path_b = ''
            self.top2_soft_labels_path_b = ''
            self.max_train_samples_b = None
            self.max_val_samples_b = None
            ...
            if self.dataset_name_b in ['EventScape_recurrent_events', 'DSEC_events', 'DDD17_events', 'E2VIDDriving_events']:
                if self.dataset_name_b == 'DSEC_events':
                    ...
                    if self.dataset_name_b == 'DDD17_events':
                        ...
                        self.delta_t_per_data_b = dataset_specs['delta_t_per_data']
                        self.pseudolabels_path_b = dataset_specs.get('pseudolabels_path_b', '')
                        if self.pseudolabels_path_b:
                            assert os.path.isdir(self.pseudolabels_path_b)
                        self.pseudolabel_confidence_path_b = dataset_specs.get('pseudolabel_confidence_path_b', '')
                        if self.pseudolabel_confidence_path_b:
                            assert os.path.isdir(self.pseudolabel_confidence_path_b)
                        self.top2_soft_labels_path_b = dataset_specs.get('top2_soft_labels_path_b', '')
                        if self.top2_soft_labels_path_b:
                            assert os.path.isdir(self.top2_soft_labels_path_b)
                        self.use_pseudolabels_train_b = bool(dataset_specs.get('use_pseudolabels_train_b', False))
                        self.pseudolabel_ignore_label_b = int(dataset_specs.get('pseudolabel_ignore_label_b', 255))
                        self.max_train_samples_b = dataset_specs.get('max_train_samples', None)
                        self.max_val_samples_b = dataset_specs.get('max_valid_samples', None)
                    else:
                ...
                if self.use_pseudolabels_train_b:
                    assert self.pseudolabels_path_b
                    assert self.require_paired_data_train_b
            ...
            self.weight_pseudolabel_loss = float(optimization.get('weight_pseudolabel_loss', 0.0))
            self.use_confidence_weighted_pseudolabel_loss = bool(
                optimization.get('use_confidence_weighted_pseudolabel_loss', False)
            )
            self.use_top2_soft_distillation = bool(optimization.get('use_top2_soft_distillation', False))
            self.weight_top2_soft_distillation = float(optimization.get('weight_top2_soft_distillation', 0.0))
            self.top2_soft_temperature = float(optimization.get('top2_soft_temperature', 1.0))
            self.top2_soft_min_p1 = float(optimization.get('top2_soft_min_p1', 0.0))
            self.top2_soft_min_margin = float(optimization.get('top2_soft_min_margin', 0.0))
            self.use_vpr_pseudo_ce = bool(optimization.get('use_vpr_pseudo_ce', False))
            self.weight_vpr_pseudo_ce = float(optimization.get('weight_vpr_pseudo_ce', 0.0))
            self.vpr_class_reliability = optimization.get('vpr_class_reliability', None)
            self.vpr_min_class_weight = float(optimization.get('vpr_min_class_weight', 0.35))
            self.vpr_max_class_weight = float(optimization.get('vpr_max_class_weight', 1.0))
            self.train_loader_len_source = optimization.get('train_loader_len_source', None)
            self.skip_validation = bool(optimization.get('skip_validation', False))
            self.task_loss = optimization['task_loss']

            if self.use_confidence_weighted_pseudolabel_loss:
                assert self.use_pseudolabels_train_b
                assert self.require_paired_data_train_b
                assert self.pseudolabel_confidence_path_b

            if self.use_top2_soft_distillation:
                assert self.use_pseudolabels_train_b
                assert self.require_paired_data_train_b
                assert self.top2_soft_labels_path_b
                assert not self.use_confidence_weighted_pseudolabel_loss
                assert self.top2_soft_temperature > 0.0
                assert self.top2_soft_min_p1 >= 0.0
                assert self.top2_soft_min_margin >= 0.0
